...
manual = sg.Frame("Manual Control",[
    ...
    sg.Text("Steps"), ITxt("50", key = "!STEPSX!", disabled=True), sg.Button("Send", key = "!SENDX!", disabled = True)],
    [sg.Text("Z Control"), sg.Radio("D", "Z", default=True, disabled=True, key="!RZD!"), sg.Radio("U", "Z", disabled=True, key="!RZU!"),
    sg.Text("Steps"), ITxt("100", key = "!STEPSZ!", disabled = True), sg.Button("Send", key = "!SENDZ!", disabled = True)],
])

calibration = sg.Frame("Calibration Constants",[
    [sg.Text("Z calibration constant [um/step]",size = (29,1)), ITxt("0.833", key = "!_Z_CONSTANT_!", disabled=True)],
    [sg.Text("Ramp to full distance ratio [%]", size = (29,1)), ITxt("33", key = "!_X_RATIO_!", disabled=True)]
])
