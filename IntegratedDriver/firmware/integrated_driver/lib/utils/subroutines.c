...
//			uint16_t default_layer_depth = frames__packFrames_uint16(USART_rx_buffer,6);	//TODO add first layer depth handler
		...
		case FRAME_NEXT_LAYER:
			if(stepper_control.device_state == READY || stepper_control.device_state == WORKING){
				stepper_control.number_of_layers = USB_buffer.val_u16_a;
		...
		switch(h_pulse.status){
			case  NOT_CALIBRATED:
				...
				if((h_pulse.current_position > H_MARGIN) && (h_pulse.lower_limit != 0))
					pulse__H_ConstPulse(H_MARGIN, 5000);
				EXTI->PR |= EXTI_PR_PR0;
...
void EXTI15_10_IRQHandler(){
	if ((EXTI->PR & EXTI_PR_PR13_Msk) != 0){
		...
		watch__reset();
//		subwatch__reset();
//		if(stepper_control.current_task == TASK_LASER_CONTROL && stepper_control.device_state == WORKING){
//			if(PWMpid.toggle == 0){
//				TIM2->CCR1 = PWMpid.out;
//				PWMpid.toggle = 1;
//			}
//			else{
//				TIM2->CCR1 = PWMpid.min_out;
//				PWMpid.toggle = 0;
//			}
//		}
//		watch__reset();
